...
# all distances
table tab_distances IN "CSV" "distances4.csv" : s <- [state1, state2], Dij~distance;

# distances from datacenters A and B, which correspond to states 
# of Florida and Oregon respectively.
table tab_florida_distances IN "CSV" "florida_distances4.csv" : ESTADOS <- [state2], DAi~distance;
table tab_oregon_distances IN "CSV" "oregon_distances4.csv" : s1 <- [state2], DBi~distance;
